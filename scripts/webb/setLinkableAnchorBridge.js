...
async function setLinkableAnchorBridge(anchorAddress, bridgeAddress) {
  const linkableAnchor = new ethers.Contract(anchorAddress, linkableAnchorContract.abi, wallet);
  await linkableAnchor.setBridge(bridgeAddress);
  console.log(`anchor at: ${anchorAddress} has bridge set to: ${bridgeAddress}`)
}

setLinkableAnchorBridge(process.argv[2], process.argv[3]);
